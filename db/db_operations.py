...
def get_objects_in_batches_from_db(model_class:Type[ConvertibleToPydantic], batch_size: int = 5000) -> List[BaseModel]:
    session = get_db_session()
    ...
    for batch in range(batches):
        ...
        random_objects = session.query(model_class)\
                                .limit(batch_size)\
                                .offset(offset)\
                                .all()
        converted_objects = [model_class.convert_from_sql_to_pydantic(obj) for obj in random_objects]
...
# def get_random_sample_annotation_areas(sample_size=5000):
#     session = get_db_session()
 
#     areas = session.query(AnnotationDB.area).order_by(func.random()).limit(sample_size).all()
#     session.close()
#     return areas
